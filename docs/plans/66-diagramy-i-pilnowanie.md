...
| Diagramy | Po krokach 63–65 istnieją cztery (układ klatki, mapa ekranów, droga klawisza, stany połączenia), jeden (cykl klatki) i jeden (start aplikacji). Przekrojowych — brak. |
| Przykłady | `examples/` objęte PHPStanem od kroku 62; wskazywane z dokumentów ścieżką i zakresem wierszy. |

## Zależności

- **Kroki 62–65** — wszystkie; ten krok pilnuje tego, co one napisały, i nie ma
  sensu przed nimi.
...
Wszystkie mermaidem, każdy ze zdaniem opisowym (konwencja z kroku 62), nazwy
węzłów **z kodu**:
...
| Cykl pętli | Wejście → stan → klatka → renderer, z fazami i tym, co w której wolno | przewodnik (krok 64) |
...
zostają ASCII-em (krok 62, punkt 5).
...
1. **`DocumentationLinksTest`** — każdy odnośnik względny w `docs/`, `README.md`
   ...
   nagłówka. To ten test, który po kroku 62 pilnuje pięćdziesięciu odnośników do
...
5. **`DocumentationExamplesTest`** — każdy przykład wskazywany z dokumentu
   ...
   PHPStan `max` i `cs-check` razem z `src/` (to już stoi od kroku 62 — test
...
rozdział w architekturze i odwrotnie (zobowiązanie z kroku 62, punkt 3).
...
- **Generowanie dokumentacji z kodu** — odrzucone w kroku 64 wraz z powodem.
- **Zrzuty ekranu odnawiane maszynowo** — wykluczone z kroku 63; jeśli wrócą, to
