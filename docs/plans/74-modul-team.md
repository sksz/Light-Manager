# Krok 74 — Moduł `team`: obraz zespołu policzony z tego, co już jest zapisane
...
Danych własnych nie ma żadnych: wkład liczy się z `git.log` (krok 66), a czas
życia prośby o scalenie — z `forge.pulls` (krok 69). To jest próba mechanizmu
...
| Źródła danych | Wyłącznie kwerendy kroków 66 i 69; własnego rozmówcy moduł nie ma. |
| Reguła 15g | Moduł pytający musi umieć żyć bez odpowiedzi — `QueryRegistry::ask()` oddaje wynik z powodem, nie `null`. |

## Zależności

- **Krok 66** — `git.log` i `git.head`; bez niego moduł nie ma treści.
- **Krok 69** — `forge.pulls`; bez niego moduł ma treść mniejszą, ale ma.
