# Krok 62 — Podręcznik użytkownika: od pierwszego uruchomienia do scenariuszy
...
- **Krok 61** — mapa dokumentacji, drzewa `pl`/`en`, konwencja diagramu
  i przykładu; bez niego nie wiadomo, gdzie ten tekst ma leżeć.
- **Kroki 21, 25, 36, 45, 48–52, 58–60** — sześć modułów, o których podręcznik
...
- **Krok 65** — testy zgodności; podręcznik jest ich pierwszym przedmiotem.
...
Spis ma postać nadającą się do porównania maszynowego (krok 65): tabela
...
kroku planu (reguła utrzymania z kroku 61, punkt 7).

### 4. Diagramy podręcznika

Cztery, wszystkie mermaidem wraz ze zdaniem opisowym (konwencja z kroku 61):
...
w kroku 63 — **do tego czasu zostają w README** i nie ma tu luki: krok 63 je
zabiera, a mapa z kroku 61 mówi, gdzie ich szukać.

### 7. Wersja angielska

Pełne tłumaczenie siedmiu plików podręcznika. **Polski jest źródłem**
(krok 61, punkt 4), więc wersja angielska powstaje po ustaleniu treści, a nie
...
- **Opis funkcji, których jeszcze nie ma** — podręcznik opisuje stan po Fazie
  ...
  **swoje kroki**, wedle reguły utrzymania z kroku 61.
- **Instrukcja dla dewelopera** — krok 63.
...
- **Spis klawiszy zgadza się ze 167 `KeyBinding`ami** — porównanie maszynowe
  wchodzi w kroku 65, ale rozbieżność wykryta wcześniej jest usterką tego kroku.
