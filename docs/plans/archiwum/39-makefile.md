...
**Ukończony** (2026-08-14). Rozstrzygnięcia startowe:
[00-decyzje.md](../00-decyzje.md), D72. Krok znalazł przy okazji dwie usterki
starsze od siebie — SIGSEGV toru okienkowego i katalogi tymczasowe po
`PinsLanguage` — obie **poza swoim zakresem** i obie **naprawione tego samego
dnia**, na osobne polecenie użytkownika (D73).
...
**Rozstrzygnięcia startowe** (dwanaście pytań, jedenaście zadanych):
[00-decyzje.md](../00-decyzje.md), D72. **Pytanie nr 5 odpadło przed zadaniem** —
krok 38 wykonał się pierwszy, więc testsuites `unit`/`functional` już były
w `phpunit.xml.dist`, a ten krok tylko dowiązał do nich cele. Zalecana kolejność
faz sprawdziła się dokładnie tak, jak ją zapisano.

**Co powstało:**

- **`Makefile`** — 26 celów w siedmiu grupach, cel domyślny `help` generowany
  z komentarzy `##` przy samych celach (spis nie jest trzecim miejscem do
  aktualizowania). `.NOTPARALLEL` globalnie: cele są cienkie, więc równoległość
  nic nie daje, a bramka ma się przewracać w znanej kolejności.
- **`bin/build-phar`** — budowa dystrybucji na klasie `Phar`, bez ani jednej
  nowej zależności Composera. Wersja z pola `version` w `composer.json`.
- **Klucze `build.*`** w `lang/pl.php` i `lang/en.php` — narzędzie budowy mówi
  przez katalog napisów, wzorem `bin/install-desktop-entry` i `bin/render-bench`.
- **Rozdział 8 „Procesy projektu”** w `docs/architecture.md` (spis „proces →
  wejście” dla dziesięciu procesów, reguła pierwszeństwa i jej granica), **reguła
  18** w `SKILL.md`, akapit w `CLAUDE.md`, `make` w README (Instalacja,
  Uruchomienie, Narzędzia deweloperskie, nowy rozdział „Budowa”, obejście
  SIGSEGV), `build/` w `.gitignore`.

**Co sprawdzono uruchomieniem** (nie „wymyślono”, zgodnie z kryterium):

| Sprawdzenie | Wynik |
|---|---|
| `make` bez argumentów | spis 26 celów; żaden nie jest ukryty |
| `make check-env` | droga zielona i droga czerwona (przez podstawiony `PHP` bez rozszerzeń): brak `imagick` daje `[FAIL]` i kod 1, brak kodera `SIXEL` — `[warn]` i kod 0 |
| `make install` na **świeżym klonie bez `vendor/`** | 63 paczki, kod 0; powtórzony nie robi nic (cel plikowy) |
| `make qa` | zielone: `cs:check`, PHPStan `max`, 1428 testów |
| `make test-unit` / `test-functional` | 1335 + 93 = 1428 — liczba po podziale zgadza się z liczbą sprzed niego |
| `make coverage` | czytelna odmowa (maszyna nie ma Xdebuga ani PCOV-u), kod 1 |
| `make install-safe` bez zmiennej | czytelna odmowa ze wzorem wywołania |
| `make build` | `build/light-manager-0.1.0.phar` (1,6 MB, 350 plików) + `build/assets/`; sprawdzenie ładowania przeszło |
| Zbudowany PHAR pod XTermem, uruchomiony **z `/tmp`** | pełna klatka, tryb Sixel — zrzut ekranu |
| `make bench`, `bench-text`, `bench-loop` | tabele wyników, obciążenie hosta 0,12–0,13 na rdzeń |
| `make bench-xterm --transfer` | faza przesyłu zmierzona (19,1 kB, 16632 kB/s, DA1 po 35,1 ms) — zrzut ekranu |
| `make run-xterm`, `run-window`, `probe-xterm` | klatka w XTermie, klatka w oknie GLFW, podgląd wejścia z odpowiedzią DA1 — zrzuty ekranu |
| `make clean` / `dist-clean` | `clean` zdejmuje `build/` i pamięci narzędzi, `dist-clean` dokłada `vendor/`; `docs/pomiary/` nietknięte |

Pomiar i okna szły na odciążonej maszynie, za potwierdzeniem użytkownika
(`CLAUDE.md`); wzorca **nie zapisywano** — krok nie dotyka potoku rysowania, więc
nie ma czego rozliczać „przed i po”.

**Odstępstwa od planu i rzeczy, które wyszły dopiero z roboty:**

1. **`bin/check-env` nie powstał** — rozstrzygnięcie nr 2 postawiło sprawdzenie
   w samym Makefile, powłoką. Tabela „Planowane zmiany w plikach” przewidywała
   ten plik warunkowo i warunek nie zaszedł.
2. **Bramki są dwie** (`qa` i `qa-full`), a plan zakładał jedną z wybranym
   zachowaniem. Różnią się jedną rzeczą: pierwsza przerywa, druga podsumowuje.
3. **`make coverage` wszedł do zakresu**, choć plan trzymał go pod znakiem
   zapytania — z czytelną odmową, bo Xdebug i PCOV nie są wymaganiami projektu.
4. **`composer.lock` wymagał odświeżenia sumy** (`composer update --lock`) po
   dopisaniu pola `version`; bez tego `composer install` ostrzega przy każdym
   wywołaniu. Zmiana obejmuje jeden wiersz pliku.
5. **Język: help po polsku, `check-env` po angielsku.** Rozstrzygnięcie nr 2
   dotyczyło komunikatów sprawdzenia środowiska (wzorem jedynego napisu poza
   katalogiem w `bin/light-manager`), a opisy celów idą rejestrem dokumentacji.
   Niespójność jest **nazwana, nie przeoczona**: `check-env` ma działać przed
   `composer install`, kiedy tłumacza nie ma; `bin/build-phar` działa po nim
   i dlatego mówi już przez katalog.
6. **`make bench-window` kończył się kodem błędu** — i to nie była wina celu.
   `./bin/render-bench --window` **sam z siebie kończył się SIGSEGV** (kod 139)
   po wypisaniu wyników, przy sprzątaniu kontekstu GLFW. Wywołanie ręczne to
   przemilczało, bo nikt nie patrzył na kod wyjścia; cel `make` przestał to
   przemilczać. Zamaskowania kodu wyjścia **nie wprowadzono z rozmysłu** — cel,
   który ukrywa naruszenie ochrony pamięci, jest gorszy od celu, który się
   przewraca.
7. **`tests/Support/PinsLanguage` zostawiał po sobie katalog w `/tmp`** przy
   każdym przebiegu (`light-manager-lang-<uniqid>`, pusty, nieusuwany). Znalezione
   przy uruchamianiu zbudowanego PHAR-a z `/tmp` — osiem takich katalogów było
   widać na liście plików.

**Obie usterki naprawione tego samego dnia**, na polecenie użytkownika, już po
zamknięciu zakresu kroku — pełny opis w [00-decyzje.md](../00-decyzje.md), D73.
Pierwsza okazała się szersza, niż wyglądała: tę samą sekwencję wykonuje
aplikacja, więc `./bin/light-manager --window` też kończył się SIGSEGV po `F10`,
tylko że niewidocznie. Zasób OpenGL ginie odtąd **przed** kontekstem
(`GlfwWindowService::releaseBeforeClose()`), a sprzątanie w testach przestało być
do zapamiętania (`#[After]` na `unpinLanguage()`). Granica „`src/` bez zmian”
dotyczyła **zakresu kroku** i została dotrzymana do jego końca; poprawki weszły
poza nim i są rozliczone osobnym wpisem.

**Granica „`src/` bez zmian” dotrzymana w całości** — i raz kosztowała: silnik
`GL\Audio` nie czyta spod `phar://`, więc w zbudowanej aplikacji domyślny utwór
nie zagra, dopóki nie wskaże się go **ścieżką bezwzględną** w ustawieniach
modułu (D72, rozstrzygnięcie 7). Budowa wypisuje tę ścieżkę na końcu, a README
opisuje rzecz w rozdziale „Budowa”.
