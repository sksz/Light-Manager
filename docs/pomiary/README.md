...
| prostokąt zaznaczenia narysowany **na oknie nakładanym** (`Marquee` nad `Dialog`iem) | `marquee-popup` | krok 77; rozlicza się **w parze z `marquee`** i tylko z nim — klatka jest ta sama, więc różnica to w całości cena tamtego kroku, a składają się na nią dwie rzeczy wynikające z odwróconej kolejności płaszczyzn: warstwa tekstowa przechodzi **także po płaszczyźnie okna** (jest `opaque`, więc `FrameText::of()` wymazuje jej prostokąt i przepisuje treścią okna), a `TextMark`i rysują się **nad** oknem, więc żaden nie chowa się pod nim. Prostokąt obrysowuje **samo okno**, a nie wiersze obok niego, bo to okno jest treścią, po którą sięga ręka — odcisk klucza hosta, wiersz logu. Daje przy tym trzecią parę za darmo (`popup` → `marquee-popup`): zaznaczenie przy oknie, tak jak `chrome-text` → `marquee` mierzy je bez okna |
...
| piąta postać ekranu modułu Dockera — **zawartość rejestru** (krok 61) | krok 61: panel rysuje `ListView` z `ListRow`ami niosącymi **samą etykietę** — bez wartości po prawej i w jednej roli — czyli **ściśle uboższą treść** niż ta, którą mierzą `text` (lista bez kursora) i `scrollbar` (lista z suwakiem). Scenariusz dałby wiersz z liczbą nieodróżnialną od tamtych dwóch, czyli **tabelę udającą pomiar**, i nie dałby się rozliczyć w parze z niczym, czego one już nie izolują. Stany zastępcze panelu („czekam”, „podaj nazwę obrazu”, powód niepowodzenia) to `Label`, mierzony przez `text`. **Odstępstwo od §7 planu kroku**, który scenariusza żądał: plan powstał, zanim panel istniał, i zakładał treść odróżnialną od listy plików — a nie jest. Cena kroku leży **poza klatką**: rozmowa z rejestrem to trzy obiegi HTTP pompowane taktem modułu, więc mierzy ją oś `--loop` |
