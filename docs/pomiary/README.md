...
| ekran książki adresowej (krok 60) | ekran **nie dowiózł ani jednego komponentu**: to `Table` o trzech kolumnach z nagłówkiem w strefie środkowej — czyli **ten sam skład**, co mierzą `columns` (kolumny i wiersze) oraz `environments` (wiersz nagłówka i role wierszy, krok 58). Okna łańcucha wpisu to `PromptOverlay` (mierzy `command`), `ChoiceOverlay` i `ConfirmOverlay` (oba `popup`), a jedna pozycja zakładki ustawień — `settings`. Krok nie zmienia ścieżki klatki poza tym, że dokłada siódmy ekran; jego cena leży **poza klatką** (odczyt sekcji dokumentu stanu przy pierwszym pytaniu o książkę) i mierzy ją tor `--loop` „przed i po” |
