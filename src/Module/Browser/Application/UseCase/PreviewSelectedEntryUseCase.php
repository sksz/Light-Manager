    ...
    private function inspect(string $path, Entry $entry): Preview
    {
        if ($entry->sizeInBytes > self::MAXIMUM_FILE_BYTES) {
            return Preview::unavailable(
                $this->translator->translate('module.browser.preview.tooLarge', [
        ...
        if ($metadata === null) {
            return Preview::unavailable($this->translator->translate('module.browser.preview.unreadable'));
        }

        if ($metadata->pixels() > self::MAXIMUM_INPUT_PIXELS) {
            return Preview::unavailable(
                $this->translator->translate('module.browser.preview.tooManyPixels', [
